...
export default function NileCruiseCard({ cruise, index = 0 }: { cruise: NileCruise; index?: number }) {
  return (
    <Link href={`/nile-cruise/${cruise.slug}`} className="block h-full">
      <motion.div
        whileHover={{ y: -10 }}
        className={`min-w-[300px] bg-white rounded-2xl overflow-hidden shadow-lg cursor-pointer group flex flex-col h-full ${cruise.isPremium ? 'border-2 border-gold/20' : ''}`}
      >
        {/* Image */}
        ...
        <div className="p-6 flex flex-col flex-1">
          ...
          <h3 className={`text-xl font-bold mb-2 transition-colors line-clamp-2 ${cruise.isPremium ? 'text-gold' : 'text-navy group-hover:text-gold'}`}>
            {cruise.title}
          </h3>
          ...
          {cruise.reviews && (
            <div className="flex items-center mb-3">
              ...
              <span className="text-sm font-bold text-navy">{cruise.reviews.rating}</span>
              <span className="text-xs text-navy/50 ml-1">({cruise.reviews.count} reviews)</span>
            </div>
          )}
          ...
          <div className="flex justify-between items-center mt-auto border-t border-gray-100 pt-4">
            ...
            <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${cruise.isPremium ? 'bg-gold text-navy' : 'bg-cream text-navy group-hover:bg-gold'}`}>
